...
declare namespace Utility {
  type GestureCheckpoint<T = number> = {
    position: T;
    time: number;
  };
}
...
function getPointerPosY(
  e: React.TouchEvent | React.MouseEvent | MouseEvent | TouchEvent
) {
  const isTouchEvent =
    ("nativeEvent" in e && e.nativeEvent instanceof TouchEvent) ||
...
export function useDraggable() {
  ...
  function onMouseMove(e: TouchEvent | MouseEvent) {
    ...
    if (isDraggable) {
      ...
      const dragProgress = Math.abs(
        parseFloat(panelElementRef.current!.style.top) / yDistance
      );
  ...
  function onMouseUp(e: TouchEvent | MouseEvent) {
    ...
    if (
      velocity > 1.4 ||
      (!isPanelOpen.current &&
        endGesture.position < calculatedGestureAreaHeight) ||
      (isPanelOpen.current && endGesture.position > calculatedGestureAreaHeight)
    ) {
      togglePanel();
    } else {
      resetPanelPosition();
  ...
  React.useEffect(() => {
    initialBorderRadius.current =
      parseFloat(
        getComputedStyle(panelElementRef.current!).borderTopLeftRadius
      ) || 0;
  ...
  React.useEffect(() => {
    ...
    const panelHeaderElementHeight =
      panelHeaderElementRef.current!.getBoundingClientRect().height;
    ...
    panelElementRef.current!.querySelector(
      "main"
